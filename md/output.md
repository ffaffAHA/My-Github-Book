...
5.2[]{style="margin-left: 1em"}arccos的区间 []{style="margin-left: 5mm"}
[15](#auto-16)
...
17.3[]{style="margin-left: 1em"}一个简单的倒带换
...
[已知特征值，求特征向量 [♥]{style="color: red"}]{style="color: red"}
...
### 4.2[]{style="margin-left: 1em"}实对称矩阵的正交规范化[]{style="margin-left: 1em"} {#auto-11}
...
## 13[]{style="margin-left: 1em"}中值定理[]{style="margin-left: 1em"} {#auto-29}
...
![](firsi-14.png){style="margin-left: -0.02256749311294766em; margin-bottom: -0.02256749311294766em; margin-right: -0.022567493112948256em; margin-top: 0em; vertical-align: 0em; height: 14.84777961432507em"}$\text{true}$

## 14[]{style="margin-left: 1em"}已知两个方程组的通解，求公共解。[]{style="margin-left: 1em"} {#auto-30}
...
## 15[]{style="margin-left: 1em"}sinx与cosx[]{style="margin-left: 1em"} {#auto-31}
...
## 16[]{style="margin-left: 1em"} {#auto-32}

微分方程

[]{style="margin-left: 1em"}

### 16.1[]{style="margin-left: 1em"}二阶，少y[]{style="margin-left: 1em"} {#auto-33}

$\begin{matrix}
 & \text{【分析】} \\
 & {y^{\prime\prime} - \frac{x + 3}{x + 1}y^{'} = 0\text{,可化为可分离变量的一阶微分方程,令
          ~}t = y^{'}.} \\
 & \text{【解答答】} \\
 & {\text{令
          ~}t = y^{'}\text{,有
          ~}y^{\prime\prime} = \frac{\text{d}y^{'}}{\text{d}x} = \frac{\text{d}t}{\text{d}x} = t^{'}.\text{原方程化为
          ~}t^{'} - \frac{x + 3}{x + 1}t = 0\text{,即}\frac{\text{d}t}{\text{d}x} = \frac{x + 3}{x + 1}t.} \\
 & {\frac{\text{d}t}{\text{d}x} = \frac{x + 3}{x + 1}t\text{满足
          ~}\frac{\text{d}t}{\text{d}x} = f(x)g(t)\text{的形式,是可分离变量的一阶微分方程。分离变量,两边积分求解。}}
\end{matrix}$

### 16.2[]{style="margin-left: 1em"}$y(x) = u(x)g(x)$的二阶微分方程[]{style="margin-left: 1em"} {#auto-34}

\[2016年真题$\rbrack_{i}$设$y(x) = u(x)e^{x}$是二阶微分方程$(2x - 1)y^{'}\prime - (2x + 1)y^{'} + 2y = 0$的解，已知$u( - 1) = e,u(0) = - 1$,已知
...
\[解答\]
由$y(x) = u(x)e^{x}$,得$y^{'}(x) = u^{'}(x)e^{x} + u(x)e^{x} = \left\lbrack u^{'}(x) + u(x) \right\rbrack e^{x}$.

[♥得到]{style="color: red"}
$(2x - 1)u^{\prime\prime}(x) + (2x - 3)u^{'}(x) = 0$ 为不显含$u$
的微分方程，

令$t = u^{'}(x)$,有$u^{\prime\prime}(x) = \frac{\text{d}u^{'}(x)}{\text{d}x} = \frac{\text{d}t}{\text{d}x} = t^{'}$.

原方程化为$(2x - 1)t^{'} + (2x - 3)t = 0$.

化为标准形式，除以$t^{'}$的系数$(2x - 1)$得$\left. t^{'} + \frac{2x - 3}{2x - 1}t = 0\Rightarrow\frac{\text{d}t}{\text{d}x} = - \frac{2x - 3}{2x - 1}t \right.$

### 16.3[]{style="margin-left: 1em"}一个简单的倒带换[]{style="margin-left: 1em"} {#auto-35}

\[2007年真题\]令微分方程$y^{\prime\prime}\left( x + y^{\prime 2} \right) = y^{'}$满足初始条件$y(1) = y^{'}(1) = 1$的特解为$y(x)$,求$y(4)$的值

\[分析\]
不显含$y$的微分方程，令$t = y^{'}(x)$,将$y$的二阶微分方程转化为$t$的一阶微分方程。

\[解答\] $令t = y^{'}(x),有$
$y^{\prime\prime}(x) = \frac{\text{d}y^{'}(x)}{\text{d}x} = \frac{\text{d}t}{\text{d}x} = t^{'}.原方程变为$
$t^{'}\left( x + t^{2} \right) = t,即$
...
$x^{'} + p(t)x = q(t)$ ,其中 $p(t) = - \frac{1}{t},q(t) = t$
,代入一阶线性微分方程的通解公式：

::::: footnote
:::: {style="margin-left: 0px" align="justify"}
...
[]{style="margin-left: 0em"} []{#footnr-1} ^[1](#footnote-1)^

### 16.4[]{style="margin-left: 1em"}高阶K重根[]{style="margin-left: 1em"} {#auto-36}
...
求$n$阶常系数线性齐次微分方程$\left. y^{(}n \right) + a_{1}(x)y^{(n - 1)} + \cdots + a_{n - 1}(x)y^{'} + a_{n}(x)y = 0$的通解：
...
[♥]{style="color: red"}$k$重实根$r$,对应$k$项$\left( C_{1} + C_{2}x + \cdots + C_{k}x^{k - 1} \right)e^{\alpha x}$;

[♥]{style="color: red"}单重复数根
$r_{1}.2 = \alpha \pm \beta i,\beta > 0$,对应两项
$e^{\alpha}x\left( C_{1}\cos\beta x + C_{2}\sin\beta x \right)$;

[♥]{style="color: red"}$k$重复数根$r_{1}.2 = \alpha \pm \beta i,\beta > 0$,对应$2k$项$e^{\alpha}x\left\lbrack \left( A_{1} + A_{2}x + \cdots + A_{k}x^{k - 1} \right)\cos\beta x + \left( B_{1} + B_{2}x + \cdots + B_{k}x^{k - 1} \right)\sin\beta x \right\rbrack$.

**例 1.** K重

已知以 y=(C~1~x+C~2~)cos 2x+(C~3~x+C~4~)sin
2x,[]{style="margin-left: 1em"}(C~1~,C~2~,C~3~,C~4~为任意常数)为通解的微分方程是
y^′′′′′^+ay^′′′^+by^′′^+cy\'+dy=0,求 a+b+c+d.

3
将$n$个根对应的所有项相加便得通解，其中$C,C_{i},A_{i},B_{i}$为任意常数。

## 17[]{style="margin-left: 1em"}定积分应用[]{style="margin-left: 1em"} {#auto-37}

### 17.1[]{style="margin-left: 1em"}旋转体体积，非$y轴,V_{} = V_{1} - V_{2}$[]{style="margin-left: 1em"} {#auto-38}

### 17.2[]{style="margin-left: 1em"}积分比大小[]{style="margin-left: 1em"} {#auto-39}
...
## 18[]{style="margin-left: 1em"}重积分[]{style="margin-left: 1em"} {#auto-40}

\[2003年真题 设$f(x) = g(x) = \left\{ \begin{matrix}
{3,} & {\text{若}0 \leq x \leq 1,} \\
{0,} & {\text{其他},}
\end{matrix}. \right.$ 而$D$
表示全平面，则$\operatorname{iint}_{D}f(x)g(y - x)$d$x$d$y =$
...
### 18.1[]{style="margin-left: 1em"}分段区间[]{style="margin-left: 1em"} {#auto-41}

令

$A = \operatorname{iint}\limits_{D}\max\left( \frac{1}{x^{2}y + 2x},\frac{1}{3x} \right)$
...
### 18.2[]{style="margin-left: 1em"}区间相同，二重积分保序性[]{style="margin-left: 1em"} {#auto-42}
...
所以$\operatorname{iint}_{D}f(x,y)$
d$\sigma > \operatorname{iint}_{D}g(x,y)$ d$\sigma$

综上选$A$.

$\begin{matrix}
 & \left. {\text{eg}\text{二重积分保序性:}}f(x,y) \geq g(x,y)\Rightarrow\operatorname{iint}\limits_{D}f(x,y)\text{d}\sigma \geq \operatorname{iint}\limits_{D}g(x,y)\text{d}\sigma. \right. \\
 & {\text{若在区域}D\text{内}f(x,y) \geq g(x,y),\text{则}\operatorname{iint}\limits_{D}f(x,y)\text{d}\sigma \geq \operatorname{iint}\limits_{D}g(x,y)\text{d}\sigma.} \\
 & \text{【证明】} \\
 & {\text{令}h(x,y) = f(x,y) - g(x,y).\text{则有}h(x,y) \geq 0,\text{则}\operatorname{iint}\limits_{D}f(x,y) - g(x,y)\text{d}\sigma = \operatorname{iint}\limits_{D}h(x,y)\text{d}\sigma \geq 0.} \\
 & {\text{即}\operatorname{iint}\limits_{D}f(x,y)\text{d}\sigma \geq \operatorname{iint}\limits_{D}g(x,y)\text{d}\sigma.}
\end{matrix}$

$\begin{matrix}
 & \\
 & {I_{3} = \operatorname{iint}\limits_{D}\cos\left( x^{2} + y^{2} \right)^{2}\text{d}\sigma,\text{其中}D = \left\{ (x,y) \middle| x^{2} + y^{2} \leq 1 \right\}\text{则}} \\
 & {A.I_{3} > I_{2} > I_{1}♥} \\
 & {\text{B.
          ~}I_{1} > I_{2} > I_{3}} \\
...
### 18.3[]{style="margin-left: 1em"}二重积分存在[]{style="margin-left: 1em"} {#auto-43}

[♥]{style="color: red"}设二元函数$f$(x,y)=xy
...
$\mid f(x,y) \mid = 0$,即$\left. \left. \lim_{(}x,y \right)\rightarrow(0,0)^{'}f(x,y) = 0 \right.$
...
## 19[]{style="margin-left: 1em"}积分表[]{style="margin-left: 1em"} {#auto-44}
...
:::: {.compact-block style="margin-bottom: 0.5em; text-indent: 0em"}
::: {align="justify"}
![](firsi-16.png){style="margin-left: -0.02256749311294766em; margin-bottom: -0.02256749311294766em; margin-right: -0.022567493112948256em; margin-top: 0em; vertical-align: 0em; height: 14.84777961432507em"}${\text{(}\text{\%o12}\text{)
          ~}}\text{true}$
:::
::::
